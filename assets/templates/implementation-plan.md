...
```markdown
### T-01 <任务标题>  [状态: ☐ / in-progress / done(日期)] ｜ [评审: ☐ / issues / passed(日期)]
...
- 验收标准:
  ...
  - [ ] 对应集成测试通过
- DoD: 质量门禁四件套（测试/全量通过/lint+类型/冒烟）+ 评审通过（标准/完整档）+ 文档同步
```
...
- DoD: lint + typecheck + 全量测试 + 冒烟 + 评审通过；蓝图 §6 API 清单同步
```
...
1. 严格按依赖顺序执行；无依赖关系的任务可并行（多会话/子代理各领一个任务）。
2. 标准/完整档每个任务走双代理：实现者子代理写码 → 评审者子代理按 references/code-review.md 审查 → 阻塞项回实现者修复（最多 2 轮）→ 评审通过才收账。迷你档可单代理实现 + 自查。
3. 每完成一个任务：勾选 + 填完成日 + 填评审状态 + 质量门禁通过。
4. 发现新需求 → 先按 Phase 1 格式进 requirements.md 拿到编号 → 再决定排进哪个里程碑。
5. 每个里程碑结束 → 向用户演示 → 收集反馈 → 必要时调整后续任务与蓝图。
